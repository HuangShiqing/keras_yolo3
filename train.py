...
# y_pred.append(Input(shape=[None, 13, 13, 3 * (5 + C)]))
# y_pred.append(Input(shape=[None, 26, 26, 3 * (5 + C)]))
# y_pred.append(Input(shape=[None, 52, 52, 3 * (5 + C)]))
#
# y_true.append(Input(shape=[None, 13, 13, 3 * (5 + C)]))
# y_true.append(Input(shape=[None, 26, 26, 3 * (5 + C)]))
# y_true.append(Input(shape=[None, 52, 52, 3 * (5 + C)]))
...
for i in range(3):
    ...
    for k in range(batch_size):
        origin_boxe = tf.boolean_mask(y_true[i][k, ..., :4], tf.cast(y_true[i][k, ..., 4], dtype=bool))
        origin_boxe = tf.tile(tf.reshape(origin_boxe, shape=[1, 1, 1, -1, 4]), [grid_w, grid_h, 3, 1, 1])
        origin_boxes.append(origin_boxe)
    ...
    loss_xy = tf.reduce_sum(
        object_mask * xywh_scale * K.binary_crossentropy(adjusted_out_xy, adjusted_true_xy,
    ...
    loss_c = tf.reduce_sum(
        object_mask * K.binary_crossentropy(adjusted_out_c, adjusted_true_c, from_logits=True) + (
                1 - object_mask) * K.binary_crossentropy(adjusted_out_c, adjusted_true_c, from_logits=True) * ignore_mask) / batch_size
    loss_class = tf.reduce_sum(
        object_mask * K.binary_crossentropy(adjusted_out_class, adjusted_true_class,
                                            from_logits=True)) / batch_size
